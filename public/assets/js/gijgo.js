...
gj.dialog.methods = {
    ...
    open: function ($dialog, title) {
        ...
        if (title !== undefined) {
            $dialog.find('[data-role="title"]').php(title);
        }
...
gj.dialog.widget = function ($element, jsConfig) {
     ...
     * @return void
     * @example Keep.php.Markup <!-- draggable, dialog.base -->
     * <div id="dialog" style="display: none">Lorem ipsum dolor sit amet, consectetur adipiscing elit...</div>
     ...
     *     }
     * </script>
     * @example Remove.php.Markup <!-- draggable, dialog.base -->
     * <div id="dialog" style="display: none">Lorem ipsum dolor sit amet, consectetur adipiscing elit...</div>
     * <button onclick="create()" class="gj-button-md">Create</button>
...
gj.grid.config = {
    base: {
         ...
         *     });
         * </script>
         * @example Remote.php.Configuration <!-- grid -->
...
gj.grid.methods = {
    ...
    renderHeader: function ($grid) {
        ...
        for (i = 0; i < columns.length; i += 1) {
            ...
            if ('checkbox' === data.selectionMethod && 'multiple' === data.selectionType &&
                ...
                $title = $('<div data-role="title"/>').php(typeof (columns[i].title) === 'undefined' ? columns[i].field : columns[i].title);
                $cell.append($title);
    ...
    useHtmlDataSource: function ($grid, data) {
        ...
        for (i = 0; i < $rows.length; i++) {
            ...
            for (j = 0; j < $cells.length; j++) {
                record[data.columns[j].field] = $($cells[j]).php();
            }
    ...
    appendEmptyRow: function ($grid, caption) {
        ...
        $wrapper = $('<div />').php(caption || data.notFoundText);
        $cell.append($wrapper);
        $row.append($cell);
    ...
    renderDisplayElement: function ($grid, $displayEl, column, record, id, mode) {
        ...
        if ('checkbox' === column.type && gj.checkbox) {
            ...
            $displayEl.php(record[column.field]);
        }
                    ...
                    if (typeof (column.editor) === 'function') {
                        ...
                        if ('checkbox' === column.type && gj.checkbox) {
                            ...
                            if ($editorField.value) {
                                $editorField.value($displayContainer.php());
                            }
                ...
                for (j = 0; j < data.columns.length; j++) {
                    ...
                    if (data.columns[j].hidden) {
                        ...
                        if (!$placeholder || !$placeholder.length) {
                            $placeholder = $('<div data-id="' + column.field + '"/>').php(tmp);
...
gj.tree.config = {
    base: {
         ...
         * @type string
         * @default 'imageUrl'
         * @example Default.php.Field.Name <!-- tree -->
         * <div id="tree"></div>
         * <script>
         *     var tree = $('#tree').tree({
         ...
         *     });
         * </script>
         * @example Custom.php.Field.Name <!-- tree -->
         * <div id="tree"></div>
         * <script>
         *     var tree = $('#tree').tree({
         *         imageUrlField: 'icon',
         ...
         * @type string
         * @default 'imageHtml'
         * @example Default.php.Field.Name <!-- materialicons, tree -->
         * <div id="tree"></div>
         * <script>
         *     var tree = $('#tree').tree({
         ...
         *     });
         * </script>
         * @example Custom.php.Field.Name <!-- materialicons, tree -->
         * <div id="tree"></div>
         * <script>
         *     var tree = $('#tree').tree({
         *         imageHtmlField: 'icon',
...
gj.tree.methods = {
    ...
    appendNode: function ($tree, $parent, nodeData, level, position) {
        ...
        if (nodeData.children.length || hasChildren) {
            ...
            data.style.leafIcon ? $expander.addClass(data.style.leafIcon) : $expander.php('&nbsp;');
        }
    ...
    update: function ($tree, id, newRecord) {
        ...
        oldRecord = newRecord;
        $node.find('>[data-role="wrapper"]>[data-role="display"]').php(newRecord[data.textField]);
...
gj.dropdown.methods = {
    ...
    open: function ($dropdown, $list) {
        ...
        $expander.php(data.icons.dropup);
    },

    close: function ($dropdown, $list) {
        var data = $dropdown.data(),
            $expander = $dropdown.parent().find('[role="expander"]');
        $expander.php(data.icons.dropdown);
        $list.hide();
    },
    ...
    select: function ($dropdown, value) {
        ...
        if (record) {
            ...
            $dropdown.next('[role="presenter"]').find('[role="display"]').php(record[data.textField]);
...
gj.datepicker.methods = {
    ...
    renderHeader: function ($datepicker, $calendar, data, date) {
        ...
        if (data.header) {
            ...
            $year.php(gj.core.formatDate(date, 'yyyy', data.locale));
            $header.append($year);
            ...
            $date.php(gj.core.formatDate(date, 'ddd, mmm dd', data.locale));
            $header.append($date);
    ...
    updateHeader: function ($calendar, data, date) {
        $calendar.find('[role="header"] [role="year"]').removeClass('selected').php(gj.core.formatDate(date, 'yyyy', data.locale));
        $calendar.find('[role="header"] [role="date"]').addClass('selected').php(gj.core.formatDate(date, 'ddd, mmm dd', data.locale));
        $calendar.find('[role="header"] [role="hour"]').removeClass('selected').php(gj.core.formatDate(date, 'HH', data.locale));
        $calendar.find('[role="header"] [role="minute"]').removeClass('selected').php(gj.core.formatDate(date, 'MM', data.locale));
    },
...
gj.datetimepicker.methods = {
    ...
    initialize: function ($datetimepicker) {
        ...
        $date.php(gj.core.formatDate(new Date(), 'ddd, mmm dd', data.locale));
        $header.append($date);
        ...
        $time = $('<div role="time" />');
        $time.append($('<div role="hour" />').on('click', gj.datetimepicker.methods.createShowHourHandler($datetimepicker, $picker, data)).php(gj.core.formatDate(new Date(), 'HH', data.locale)));
        $time.append(':');
        $time.append($('<div role="minute" />').on('click', gj.datetimepicker.methods.createShowMinuteHandler($datetimepicker, $picker, data)).php(gj.core.formatDate(new Date(), 'MM', data.locale)));
        $switch.append($time);
